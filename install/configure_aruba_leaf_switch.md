...
Requirements:
    - Two uplinks from the switch to the upstream switch, this is can be an leafregation switch or a spine switch.
...
The uplinks are port 49 and 50 on the leaf. They connect to leafregation switch 1 and 2 on port 47.
...
1. The following configuration is applied to Aruba leaf/leafregation switches.
   ```
   sw-leaf-001(config)#
