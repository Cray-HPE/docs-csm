...
1. Start a new typescript (quit).
   ...
   > **NOTE:** Run this on the PIT node as root. The prompts are removed for easier copy-paste. This step is only useful as a whole.
...
1. Upload the bootstrap information; note that this denotes information that should always be kept together in order to fresh-install the system again.
    ...
    2. Create the backup by logging in to `ncn-m002` and `ncn-m003` and pulling the files off the PIT. _This runs `rsync` with specific parameters; `partial`, `non-verbose`, and `progress`._
...
1. Exit the typescript and move the backup over to `ncn-m001`, thus removing the need to track `ncn-m002` as yet-another bootstrapping agent. This is required to facilitate reinstallations, because it pulls the preparation data back over to the documented area (`ncn-m001`).
