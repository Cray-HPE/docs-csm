...
1. Make sure the OSDs (if any) are not running after running the previous command.
...
**IMPORTANT:** For each step, oay attention to whether the command is to be run on a master node, storage node, or worker node. If
...
1. Unmount volumes.
    ...
    The exact umount command used depends on the node type:
    ...
    * **Storage nodes**
        ...
        If the `umount` command is responding with `target is busy` on the storage node, then try the following:
            ...
            1. Umount `/var/lib/containers/storage/overlay`.
