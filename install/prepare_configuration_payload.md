...
passing command-line arguments to CSI, as well as the creation of a number of
...
* Several parameters (`cmn-gateway`, `cmn-cidr`, `cmn-static-pool`, `cmn-dynamic-pool`) describe the CMN (Customer Management network).
  ...
  * The `cmn-external-dns` is the static IP address assigned to the DNS instance running in the cluster to which requests the cluster subdomain will be forwarded.
    * The `cmn-external-dns` IP address must be within the `cnn-static-pool` range.
...
* For systems that use non-sequential cabinet ID numbers, use `cabinets-yaml` to include the `cabinets.yaml` file.
  * This file can include information about the starting ID for each cabinet type and number of cabinets which have separate command line options.
  * It also is a way to specify explicitly the ID of every cabinet in the system.
