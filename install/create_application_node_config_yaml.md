...
1. Create a file called `application_node_config.yaml` with the following contents. This is a base application node config file for CSI that does not add any additional prefixes, HSM SubRole mappings, or aliases.
...
6. Verify the final information in the example `application_node_config.yaml` built from the [example hmn_connections.json output](#hmn-connections-example-output).
