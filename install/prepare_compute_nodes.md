...
1. [Configure HPE Apollo 6500 XL645d Gen10 Plus Compute Nodes](#configure-hpe-apollo-6500-x645d-gen10-plus-compute-nodes)
    ...
    1. [Configure the iLO to use VLAN 4](#configure_ilo)
    1. [Configure the switch port for the iLO to use VLAN 4](#configure_switch_port)
    1. [Clear bad MAC and IP address out of KEA](#cleanup_kea)
    1. [Clear bad ID out of HSM](#cleanup_hsm)
...
There may not be a `10.254` address associated with the node -- that is OK and
...
1. After accepting the BMC restart, connection to the BMC will be lost until the switch port reconfiguration is performed.
...
1. SSH to the switch and log in with standard admin credentials. Refer to `/etc/hosts` for exact hostname.
...
1. Configure the port if the MAC address is correct.
...
1. Remove the entry from KEA that is associated with the MAC address and IP address gathered previously.
...
The BIOS time for Gigabyte compute nodes must be synchronized with the rest of the system.
