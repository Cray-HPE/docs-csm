...
These steps provide a bootable USB with SSH enabled, capable of installing this Shasta release.
...
1. Confirm that the IPMI credentials work for the BMC by checking the power status.
   ...
   ```bash
   external# USERNAME=root ; SYSTEM_NAME=eniac
   external# read -s IPMI_PASSWORD
   external# export IPMI_PASSWORD ; ipmitool -I lanplus -U $USERNAME -E -H ${SYSTEM_NAME}-ncn-m001-mgmt chassis power status
   ```
