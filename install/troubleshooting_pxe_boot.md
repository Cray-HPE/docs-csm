...
This page covers issues that arise when PXE booting nodes in an HPE Cray EX system.
...
- `spine01` and `spine02` need an active gateway on VLAN 1; this can be identified from `MTL.yaml` generated by
...
Restart the [Boot Script Service (BSS)](../glossary.md#boot-script-service-bss) if the following output is returned on the console
during an [NCN](../glossary.md#non-compute-node-ncn) PXE boot attempt (specifically the `404 Not Found` error at the bottom):
...
If the PXE boot is giving 404 errors, this could be because the necessary information is not in [BSS](../glossary.md#boot-script-service-bss). The
...
1. Reboot to the [PIT](../glossary.md#pre-install-toolkit-pit).

    - **If using a remote ISO PIT**, then skip to the next step.
    - **If using a USB PIT**, then follow this procedure:
...
1. (`pit#`) Set variables for the system name, the [CAN](../glossary.md#customer-access-network-can) IP address for `ncn-m002`, the Kubernetes version, and the Ceph version.
