...
If SNMP is mis-configured, REDS Hardware Discovery and the Prometheus SNMP Exporter may
...
If the passwords do not match, either update customizations.yaml to match the switches or change the switches to match
 customizations.yaml.  Procedures with both options can be found on the [Configure SNMP](configure_snmp.md) page.
...
factory defaults).  To persist the SNMP settings a [CANU custom configuration](./canu/custom_config.md) should
...
Create a [CANU custom configuration](./canu/custom_config.md) that configures SNMP on the management network switches,
...
See the [Configure SNMP](configure_snmp.md) page for more information about configuring SNMP in CSM.
