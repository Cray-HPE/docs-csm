...
3. Expand the CSM software release:
   ...
   > 
   > **`WARNING`** Ensure that the `CSM_RELEASE` environment variable is set to version of the patched CSM release tarball. 
   > After the "CSM Patch Assembly" procedure is applied it will result in a different CSM version when compared to 
   > pre-patched CSM release tarball.
   ```bash
