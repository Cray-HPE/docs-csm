...
During some node rebuilds, the SLS Postgres database gets into a bad state, causing SLS to become unhealthy. This page outlines how to detect if
this has happened and provides a remediation procedure.
...
* If the `STATUS` is `SyncFailed`, then see [Postgres status `SyncFailed`](../../operations/kubernetes/Troubleshoot_Postgres_Database.md#postgres-status-syncfailed).
* Otherwise, consult the standard Postgres troubleshooting procedures for further avenues of investigation. See
