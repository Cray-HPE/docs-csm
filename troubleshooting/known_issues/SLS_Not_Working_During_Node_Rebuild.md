...
CSM version (1.7) and for the previous CSM version (1.6).
...
* If the `STATUS` is `SyncFailed`:
    * If currently doing a CSM upgrade, then see
      [(CSM 1.6) Postgres status `SyncFailed`](https://github.com/Cray-HPE/docs-csm/blob/release/1.6/operations/kubernetes/Troubleshoot_Postgres_Database.md#postgres-status-syncfailed).
...
* Otherwise, see the standard Postgres troubleshooting procedures for further avenues of investigation.
    * If currently doing a CSM upgrade, then see
      [(CSM 1.6) Troubleshoot Postgres Database](https://github.com/Cray-HPE/docs-csm/blob/release/1.6/operations/kubernetes/Troubleshoot_Postgres_Database.md).
