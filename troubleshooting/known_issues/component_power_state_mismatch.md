# SAT/HSM/PCS Component Power State Mismatch

Because of various hardware or communication issues, the node state reported by SAT and HSM (Hardware State Manager) may
become out of sync with the actual hardware state reported by PCS or Redfish. In most cases this will be noticed
...
Any power operations done manually with PCS that alter the nodes' power state may also cause the power state to re-sync.
