...
Due to the way the resolver code works in certain versions of Alpine Linux, it may be necessary to reconfigure the `cray-spire-postgres-pooler` to use the fully qualified domain name of the database in order to prevent DNS lookup errors.

## Symptoms

* The `cray-spire-server` pods are logging `query_wait_timeout` errors.
...
* The `cray-spire-postgres-pooler` pods are logging DNS lookup failure errors.

  ```text
  2022-11-15 09:38:40.290 UTC [1] WARNING DNS lookup failed: cray-spire-postgres: result=0
  2022-11-15 09:38:56.211 UTC [1] WARNING DNS lookup failed: cray-spire-postgres: result=0
  2022-11-15 09:39:11.881 UTC [1] WARNING DNS lookup failed: cray-spire-postgres: result=0
  2022-11-15 09:39:27.879 UTC [1] WARNING DNS lookup failed: cray-spire-postgres: result=0
...
**IMPORTANT:** This change will need to be reapplied if the `cray-spire` Helm chart is re-installed.
