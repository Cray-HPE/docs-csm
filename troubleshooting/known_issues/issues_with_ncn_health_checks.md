...
- [Specific test issues](#specific-test-issues)
    - [`Kubernetes Query BSS Cloud-init for ca-certs`](#kubernetes-query-bss-cloud-init-for-ca-certs)
    - [`Kubernetes Velero No Failed Backups`](#kubernetes-velero-no-failed-backups)
    - [`Verify spire-agent is enabled and running`](#verify-spire-agent-is-enabled-and-running)
    - [`cfs-state-reporter service ran successfully`](#cfs-state-reporter-service-ran-successfully)
    - [Clock skew test failures](#clock-skew-test-failures)
    - [`Etcd backups missing after system power up`](#etcd-backups-missing-after-system-power-up)

## General issues

- During an install of CSM, the first pass of running these tests may fail because `cloud-init` has not
  completed on the storage nodes. In the case of failure, wait for five minutes and rerun the tests.

- Some of the tests will fail if the
  [Cray CLI](../../glossary.md#cray-cli-cray) is not configured on the
  [management NCNs](../../glossary.md#management-nodes).
...
Because of a [known issue  with Velero](https://github.com/velero-io/velero/issues/1980), a backup may be attempted immediately
...
For more information on the [CFS](../../glossary.md#configuration-framework-service-cfs) State Reporter
service, see [CFS State Reporter](../../operations/configuration_management/CFS_State_Reporter.md).
