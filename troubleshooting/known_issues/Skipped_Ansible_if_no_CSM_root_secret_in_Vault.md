...
[Configure Non-Compute Nodes with CFS](../../operations/CSM_product_management/Configure_Non-Compute_Nodes_with_CFS.md).
...
1. Find the latest CSM install on the system.
...
1. Set a variable for the `configuration.import_branch` value from the most recent installed version,
   in this example `cray/csm/1.27.2`.

    ```bash
    ncn-mw# CSM_IMPORT_BRANCH="cray/csm/1.27.2"
    ```

1. Check out the `csm-config` Ansible plays.
    ...
    ncn-mw# git clone https://$VCS_USER:$VCS_PASSWORD@api-gw-service-nmn.local/vcs/cray/csm-config-management.git
    ```
...
1. Set up a temporary branch based on the branch for the installed version of CSM.

    ```bash
    ncn-mw# cd csm-config-management
    ncn-mw# git checkout $CSM_IMPORT_BRANCH
    ncn-mw# git checkout -b patch_branch
    ```
...
1. Modify the `csm.ssh_keys` role to include
...
1. Commit and push the changes.
    ...
    ```bash
    ncn-mw# git commit -a -m "Remove inappropriate calls to end_play in csm.ssh_keys role"
    ncn-mw# git push -u origin patch_branch
    ```
...
1. Record the new commit ID for the patch changes.
