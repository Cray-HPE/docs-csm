...
2. Configure the LTSS repo for image customization in `vars/uan_repos.yaml` in the `uss-config-management` VCS repo:
...
**Note:** This step is required untill the USS fix to include LTSS repo in the base image recipe which will be delivered
in USS 1.3.7. This step can be skipped if USS 1.3.7 or higher version is installed. 

3. Restart the IMS pod to pick up the configmap change:
