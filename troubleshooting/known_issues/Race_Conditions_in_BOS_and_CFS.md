...
CFS includes operators that run separately from the API server:
the CFS batcher, the CFS hardware synchronization agent, and the CFS operator.
These operators generally work in a loop where they make a query to the
