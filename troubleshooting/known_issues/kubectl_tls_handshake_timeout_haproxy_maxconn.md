...
(`ncn-m#`) Check the number of `kubectl` processes running on all master and worker nodes. A healthy node should have only a
...
(`ncn-m#`) Run the following to terminate the stuck processes. Replace `ncn-w0[01-XX]` with the appropriate
