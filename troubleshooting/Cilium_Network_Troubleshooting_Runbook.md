...
CSM uses Cilium Container Network Interface (CNI) plugin. The plugin is responsible for assigning IP
addresses to pods and establishing network connectivity within and between Kubernetes nodes.
...
- [Inspect Cilium logs](#inspect-cilium-logs)
- [Cilium monitoring](#cilium-monitoring)
- [Troubleshooting using Hubble](#troubleshooting-using-hubble)
...
All the pods should have `Running` status and the number of `Ready` pods should match the `Desired` count.
...
1. (`ncn-mw#`) Get the Cilium status for a particular node.

    > In this example, the pod `cilium-dlp8x` is running on node `ncn-w004`, and it is used to
    > check the status of Cilium on that node.
...
Cilium monitoring helps to identify issues with network policies, connectivity, and more.
Hubble can also be used for more advanced monitoring and troubleshooting.
See the [Cilium and Hubble Monitoring](../operations/system_management_health/Cilium_Monitoring.md) documentation for more information.

## Troubleshooting using Hubble

1. (`ncn-mw#`) To use the Hubble tool to diagnose network problems start a port-forwarding session.
...
1. (`ncn-mw#`) Use the `hubble` command to inspect network traffic and events.

    > This example looks at traffic that is being dropped because of network policies.
