...
1. For a particular Postgres cluster, the expected output is similar to the following:
    ...
    For each Postgres cluster:
      ...
      - Verify there is one cluster member with the Leader Role and log output indicates expected status. Such as:
         ...
         ```
         Errors reported prior to the lock status, such as **ERROR: get_cluster** or **ERROR: ObjectCache.run ProtocolError('Connection broken: IncompleteRead(0 bytes read)', IncompleteRead(0 bytes read))** or **ERROR: failed to update leader lock** can be ignored.
