...
- [Pre-install CSM](#pre-install-csm)
- [Post-install CSM](#post-install-csm)
...
1. Obtain the list of site-specific UAN NMN IP addresses.

1. (`pit#`) Update `customizations.yaml` with the list of UAN node IP addresses.
...
| `endpoints`   | `10.252.1.13` | List of UAN NMN IP addresses to monitor SMARTMON data                      |
...
1. (`uan#`) Obtain the list of UAN NMN IP addresses.
...
1. (`ncn-mw#`) Update `customizations.yaml` with the list of UAN node IP addresses.
...
1. (`ncn-mw#`) Review the UAN Node Exporter configuration.
...
1. (`ncn-mw#`) Verify that the changes are in place.
