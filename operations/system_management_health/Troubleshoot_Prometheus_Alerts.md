...
Alerts for `PostgresqlHighRollbackRate` on `spire-postgres`, `smd-postgres`, and `keycloak-postgres` pods can be ignored. This is caused by an idle session that requires a timeout.
