...
This is a `VictoriaMetrics` Exporter for extracting metrics from a server using the Redfish API.
The hostname of the server has to be passed as target parameter in the HTTP call.

> ***NOTE:*** This is only compatible with `NEO 7.2` and below because of Redfish API design changes.
...
In order to provide SMART data to the `victoria-metrics` time series database,
...
1. In `.spec.template.spec.containers.`, add the `-d` option and save the file.
...
> ***NOTE:*** Now `kubectl logs -n sysmgmt-health <redfish-exporer pod name>` will show more detailed logging.
