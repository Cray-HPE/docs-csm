...
1. (`linux#`) Check the status of `cfs-state-reporter`.
...
(`linux#`) In this case, try restarting the service, to see if it resolves the problem.
...
* If the log messages indicate problems communicating with Spire, checking the health of the Spire
  ...
  [Troubleshooting Spire](../../troubleshooting/README.md#spire) and
  [Spire operational information](../README.md#spire).

* (`ncn-mw#`) If there are errors indicating failure to communicate with the
