...
1. When a node reboots, the CFS State Reporter runs on the node, and contacts the CFS API to enable and clear the state for the
...
1. The CFS Batcher monitors the CFS API, periodically querying for enabled components with a `pending` configuration status.
