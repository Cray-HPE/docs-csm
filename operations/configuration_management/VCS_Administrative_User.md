...
The following command retrieves and decodes the VCS administrative user password from the `vcs-user-credentials` secret.
...
1. Identify the Gitea/VCS pod.
    ...
    ```bash
    ncn-mw# POD=$(kubectl get pods -n services -l app.kubernetes.io/instance=gitea --no-headers | grep -w Running | awk '{ print $1 }') ; echo ${POD}
    ```
...
1. Open an interactive shell into the pod.

    ```bash
    ncn-mw# kubectl exec -it -n services "${POD}" -- sh
    ```

1. Update Gitea/VCS with the new password.
...
* When reaching the step to update customizations, perform the following steps:
    ...
    **Only follow these steps as part of the previously linked chart redeploy procedure.**
    ...
    1. Run `git clone https://github.com/Cray-HPE/csm.git -b release/1.2`.
    ...
    1. Change the password in the `customizations.yaml` file.
        ...
        1. Delete the current Gitea sealed secret data from `customizations.yaml`.
            ...
            ```bash
            ncn-mw# yq delete -i "${CUSTOMIZATIONS}" spec.kubernetes.sealed_secrets.gitea
            ```
        ...
        1. Verify that the Gitea sealed secret data is not present.
            ...
            ```bash
            ncn-mw# yq r "${CUSTOMIZATIONS}" spec.kubernetes.sealed_secrets.gitea
            ```
        ...
        1. Update `customizations.yaml` with the new password.
            ...
            ```bash
            ncn-mw# yq w -i "${CUSTOMIZATIONS}" -f new_gitea_pw.yaml spec.kubernetes.sealed_secrets.gitea.generate
            ```
        ...
        1. Verify that updates were successfully applied.
            ...
            ```bash
            ncn-mw# yq r "${CUSTOMIZATIONS}" spec.kubernetes.sealed_secrets.gitea.generate
            ```

        1. Delete `new_gitea_pw.yaml`.

            ```bash
            ncn-mw# rm new_gitea_pw.yaml
            ```
    ...
    1. Encrypt the values after changing the `customizations.yaml` file.
        ...
        ```bash
        ncn-mw# ./utils/secrets-seed-customizations.sh "${CUSTOMIZATIONS}"
        ```

        If the above command complains that it cannot find `certs/sealed_secrets.crt`, then run the following commands to create it:

        ```bash
        ncn-mw# mkdir -pv ./certs && ./utils/bin/linux/kubeseal --controller-name sealed-secrets --fetch-cert > ./certs/sealed_secrets.crt
        ```
...
* When reaching the step to validate that the redeploy was successful, perform the following steps:
    ...
    **Only follow these steps as part of the previously linked chart redeploy procedure.**
    ...
    1. Do a rolling restart of the Gitea/VCS service and wait for it to complete.
        ...
        ```bash
        ncn-mw# kubectl rollout restart -n services deployment gitea-vcs && kubectl rollout status -n services deployment gitea-vcs && echo SUCCESS || echo ERROR
        ```
    ...
    1. Run a VCS health check.
        ...
        ```bash
        ncn-mw# /usr/local/bin/cmsdev test -q vcs
        ```
