...
This example, uses Kubernetes secrets and assumes that the Broker UAIs run in the `uas` Kubernetes namespace. If a different namespace is used, the creation of the ConfigMaps is different but the contents are the same. Using a namespace other than `uas` for Broker UAIs has implications beyond secrets and configmaps, is not recommended and is beyond the scope of this document.
