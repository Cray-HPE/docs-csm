...
The steps in this document must also be completed immediately prior to
...
4. Run the `fru_history_remove_duplicate_detected_events.sh` pruning script
   ...
   possibly over 24 hours.  While you can certainly move forward with
   this section, we recommend that you page down and follow the
   [Alternative Procedure for Large Databases](#alternative-procedure-for-large-databases)
   section. Breaking the operation up into chunks facilitates
...
Skip this procedure if you ran the procedure above and step 4 is completed
successfully.

Follow this procedure if step 4 in the procedure above failed,
or if you are pruning events on a large system with more than
1500 nodes.
