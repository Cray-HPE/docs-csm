...
* Retrieve just the members array for a group:
    ...
    ```screen
    ncn-mw# cray hsm groups members list GROUP_LABEL  --format json
    ```
...
* Retrieve only the members of a group that are also in a specific partition:
    ...
    ```screen
    ncn-mw# cray hsm groups members list --partition PARTITION_NAME GROUP_LABEL
    ```

* Retrieve only the members of a group that are not in any partition currently:

    ```screen
    ncn-mw# cray hsm groups members list --partition NULL GROUP_LABEL
    ```
...
```screen
ncn-mw# cray hsm groups members create --id MEMBER_ID GROUP_LABEL
```

For example:

```screen
ncn-mw# cray hsm groups members create --id x1c0s0b0n0 blue
```
...
```screen
ncn-mw# cray hsm groups members delete MEMBER_ID GROUP_LABEL
```

For example:

```screen
ncn-mw# cray hsm groups members delete x1c0s0b0n0 blue
```
