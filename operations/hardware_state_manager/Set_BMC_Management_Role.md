...
The BMCs of NCNs should be marked with the Management role as early as possible in the install/upgrade cycle, in order to prevent unintentionally taking down a critical node. The Management role on the BMCs cannot be set until after Kubernetes is running and the HSM service is operational.
