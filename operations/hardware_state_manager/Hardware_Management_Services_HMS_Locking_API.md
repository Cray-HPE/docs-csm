...
power state change with the [Power Control Service (PCS)][pcs] or
[Cray Advanced Platform Monitoring and Control (CAPMC)][capmc].

Locks only constrain FAS and PCS/CAPMC from each other, and help ensure that a firmware
...
> **Note**: Starting in CSM 1.7, the [Boot Orchestration Service (BOS)][bos] is
...
Locks can only be used to prevent firmware updates with FAS or power state changes with CAPMC/PCS.
...
perform a firmware update via FAS or power state change via PCS/CAPMC cannot be made on that
...
FAS or PCS/CAPMC could power off the NCNs, which may negatively impact system stability and the
