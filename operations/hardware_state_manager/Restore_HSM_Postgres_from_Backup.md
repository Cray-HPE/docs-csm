# Restore HSM (Hardware State Manager) Postgres Database from Backup
...
6. Determine the database schema version of the currently running HSM database and verify that it matches the database schema version from the Postgres backup:
    ...
    __WARNING__: If the database schema versions do not match the version of the HSM deployed will need to be either upgraded/downgraded to a version with a compatible database schema version. Ideally to the same version of HSM that was used to create the Postgres backup.
