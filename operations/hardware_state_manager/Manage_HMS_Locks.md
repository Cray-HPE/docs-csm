...
the Firmware Action Service (FAS) is aware that it has lost the reservation. Additionally, if PCS has
a reservation that is cancelled, disabled, or broken, it will do nothing to the existing PCS operation.
There are no checks by PCS to make sure things are still reserved at any time during a power operation.
