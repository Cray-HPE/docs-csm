...
The configuration documented in this procedure is intended to prevent user-facing dedicated nodes (UANs, Compute Nodes) for retrieving NCN image content from Ceph S3 Services, as running on storage nodes. Specifically, the controls enacted via this procedure should:
...
The procedure was tested on a CSM 1.2.0 GA System. 

## Test Connectivity Before Applying the ACL

Save the following script to a file (e.g., con_test.sh). 
...
## Configure HAProxy ACLs and Logging

Necessary to limit RGW VIP access, to the ncn-images bucket, to management NCNs. 

### Build IP address list of NCNs, on NMN
...
### Create a new rsyslog configuration for haproxy, to have it listen to UDP 514 on localhost
...
## Apply server-side iptables rules to storage nodes, need to execute for each storage node
...
### Document where RadosGW is running (port wise)
...
### List iptables again, verify rules are in place
...
## Validate that you cannot connect to HAProxy for ncn-images or Ceph RADOS GW (at all) from compute and UAN nodes
...
# curl --connect-timeout 2 rgw-vip:8080
...
```
# pdsh -N -w ncn-s00[1-4] "cd /var/log && zgrep -h -i -E 'haproxy.*frontend' messages || exit 0" | grep "ncn-images" | grep "10.252.1.13"
...
In the firewall logs, you should see Ceph RADOS GW traffic being dropped on the storage node, e.g.:

```
# pdsh -N -w ncn-s00[1-4] "grep RADOSGW /var/log/firewall" | grep "10.252.1.13" | head -1
