...
For more information on Gitea/VCS, including how to change the password, see [Version Control Service (VCS)](../configuration_management/Version_Control_Service_VCS.md).
...
The tables below include the default login credentials for each rack type.
These passwords can be changed by going into the console on a given switch and changing it.
...
In both cases, a `running-config` must be saved out to non-volatile storage in a startup configuration file. If it is not, the password will revert to default on the next boot.
This is identical behavior as standard managed Ethernet switches.
...
- Change the global default credential on HPE Cray EX liquid-cooled cabinet embedded controllers (BMCs).
   ...
   See [Change Cray EX Liquid-Cooled Cabinet Global Default Password](../security_and_authentication/Change_EX_Liquid-Cooled_Cabinet_Global_Default_Password.md)
