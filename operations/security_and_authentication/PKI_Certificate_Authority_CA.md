...
On NCNs, trusted certificates are installed by `Cloud-init` in the `/etc/pki/trust/anchors/platform-ca-certs.crt` file.
Refer to [Make HTTPS Requests from Sources Outside the Management Kubernetes Cluster](Make_HTTPS_Requests_from_Sources_Outside_the_Management_Kubernetes_Cluster.md)
for more information for clients that are outside the system's management cluster.
