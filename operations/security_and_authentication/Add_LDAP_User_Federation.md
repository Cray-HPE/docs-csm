...
1. (Optional) (`ncn-mw#`) Add the LDAP CA certificate in the `certs.jks` section of `customizations.yaml`.
   ...
   1. Update `kafka` secrets with the new `certs.jks.b64`.
      ...
      ```bash
      kubectl get secret cray-shared-kafka-clients-ca-cert -n services -o json | jq --rawfile cert certs.jks.b64 '.data["ca.p12"]=$cert' | kubectl apply -f -
      kubectl get secret cray-shared-kafka-cluster-ca-cert -n services -o json | jq --rawfile cert certs.jks.b64 '.data["ca.p12"]=$cert' | kubectl apply -f -
      kubectl get secret cray-shared-kafka-clients-ca-cert -n services -o json | jq -r '.data["ca.p12"]'
      kubectl get secret cray-shared-kafka-cluster-ca-cert -n services -o json | jq -r '.data["ca.p12"]'
      ```
