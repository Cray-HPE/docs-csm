...
1. (Optional) Add the LDAP CA certificate in the `certs.jks` section of `customizations.yaml`.
   ...
   1. Load the `openjdk` container image.
      ...
      ```bash
      ncn-mw# ${CSM_DISTDIR}/hack/load-container-image.sh artifactory.algol60.net/csm-docker/stable/docker.io/library/openjdk:11-jre-slim
      ```
   ...
   1. Create (or update) `cert.jks` with the PEM-encoded CA certificate for an LDAP host.
      ...
      ```bash
      ncn-mw# podman run --rm -v "$(pwd):/data" artifactory.algol60.net/csm-docker/stable/docker.io/library/openjdk:11-jre-slim keytool \
   ...
   1. Create `certs.jks`.
      ...
      ```bash
      ncn-mw# podman run --rm -v "$(pwd):/data" artifactory.algol60.net/csm-docker/stable/docker.io/library/openjdk:11-jre-slim keytool -importcert \
