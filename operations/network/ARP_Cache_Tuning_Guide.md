...
If the environment uses IPv6, administrators may also want to set the equivalent IPv6 neighbor cache settings.
Just like the IPv4 ARP cache, IPv6 maintains a neighbor cache table to map IPv6 addresses to MAC addresses.
...
1. (`ncn-mw#`) Update the appropriate variables in `roles/csm.ncn.sysctl/vars/main.yml` using the values calculated
   ...
   > **NOTE** If the IPv6 neighbor cache table is also required for a system, then add them to the `sysctl_config` list as well.
   > See [IPv6 considerations](#ipv6-considerations) for more information.
...
1. (`ncn-m001`) Verify the IPv6 neighbor table cache settings (if configured).
