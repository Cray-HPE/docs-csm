...
1. On both core switches create `loopback1` interface using the same IP address for both devices.
1. Enable OSPF on loopback interface and make sure route redistribution is configured.
1. Enable `PIM-SM` on `loopback1`
1. Configure `loopback1` to act as RP for both `8325s` using: `rp-candidate source-ip-interface loopback1`
1. Configure both core devices to advertise the same specific multicast subnet (which will be used later) using: `rp-candidate group-prefix 239.1.1.0/24`
1. Enable BSR on both routers using: `bsr-candidate source-ip-interface loopback0` in the router `pim` context.
