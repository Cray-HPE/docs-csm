...
```bash
ip route replace default via 172.30.48.1
```
...
```bash
canu validate switch config --running ./1.0/sw-spine-002.cfg --generated ./1.2/sw-spine-002.cfg --vendor mellanox --remediation
```
...
```ShellSession
cat sw-spine-002.cfg| grep "interface vlan 7"
interface vlan 7 vrf forwarding Customer
...
```bash
canu validate switch config --running 1.0/sw-leaf-bmc-001.cfg --generated 1.2/sw-leaf-bmc-001.cfg --vendor dell --remediation
+ ip vrf Customer
+ interface vlan7
+   description CMN
+   no shutdown
...
```bash
grep -A 7 "interface vlan 7" sw-spine-002.cfg
interface vlan 7
    vrf attach Customer
    description CMN
    ip mtu 9198
