...
**IMPORTANT:** All of these steps should be done using an out of band connection. This process is disruptive and will require downtime. 
...
This should only need to be done once (Unless hardware is added or PoR config has changed).
Once this configuration file has been saved, the user should be able to switch between the two configuration files.

The following example is a 1.0 system that is moving to 1.2.

1. Write current configuration to file, this copies the current running config to a binary config file.
    
    ```
    (config) # configuration write to csm1.0
    ```
    
    This should create a new configuration file.

1. Verify a new configuration file was created.
...
1. Create a new config file for csm 1.2, when a new config file is created no data is written to it. We will boot to this new config file and paste the CANU generated config to it.
...
1. Once that is completed, save the config.
...
In this example, the config is going from a CSM 1.2 to CSM 1.0 switch config.
...
1. Switch to desired config version.
...
1. Verify the switch is booted to the specified config version.
...
### Save a configuration file from a running system (going from one CSM release to the other for the first time)

This should only need to be done once (unless hardware is added or PoR config has changed). Once this configuration file has been saved, users should be able to switch between the two configuration files.

This example is for a 1.0 system that is going to 1.2.
...
1. Paste in the new CANU generated config.

1. Save the config and verify that both configs exist.
...
This process should be used when config files for the desired CSM version are currently on the switch.
The following example is for going from a CSM 1.2 to CSM 1.0 switch config.
...
### Save the current configuration into a checkpoint (needs be done when in both CSM1.0 and CSM1.2 configuration to save the configs in checkpoint)
...
1. Copy the existing checkpoint point to the startup config to switch between the CSM 1.0 and CSM 1.2 configuration.
    
    1. Copy the checkpoint to startup:
