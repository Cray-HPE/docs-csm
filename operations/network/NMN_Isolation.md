...
- ACLs preventihg Mountain compute cabinets (EX) from communicating with each other
- PVLAN to prevent River managed nodes (compute, UAN, etc...) from communicating with each other

NMN Isolation alleviates the need for most host-based iptables on management nodes (managers, workers, and storage NCN). The PVLAN sub-feature also removes the need for host-based iptables on UAN (and other River managed nodes) by limiting access over the NMN.
...
- `canu test` runs a full diagnostic on all managment network switches, including tests for NMN Isolation ACLs
...
5. Retrieve any switch custom configurations file used in previous installations or upgrades, often named [custom_config.yaml](management_network/canu/custom_config.md). This file includes any site network customizations, including uplinks to site networks, SNMP configurations, or port configurations that are not generated as part of CANU.
...
As noted previously, NMN Isolation consists of three subfeatures.  These are listed and shown in the diagram below.
...
Managed nodes are limited to access only CSM services on the management nodes. The ACLs of this subfeature are named `MANAGED_NODE_ISOLATION` and replace the existing `nmn-hmn` ACL on the NMN.  The list of allowed services is as follows:
