...
- (`ncn-mw#`) Healthy Postgres cluster.
...
- Previously taken backup of the SLS Postgres cluster (either a manual or automatic backup).
    ...
    (`ncn-mw#`) Check for any available automatic SLS Postgres backups:
...
1. (`ncn-mw#`) Retrieve a previously taken SLS Postgres backup.
    ...
    - From a previous manual backup:
        ...
        1. Set the environment variable `POSTGRES_SQL_FILE` to point toward the `.psql` file in the backup folder:
            ...
            ```bash
            POSTGRES_SQL_FILE=/root/cray-sls-postgres-backup_2021-07-07_16-39-44/cray-sls-postgres-backup_2021-07-07_16-39-44.psql
            ```

        1. Set the environment variable `POSTGRES_SECRET_MANIFEST` to point toward the `.manifest` file in the backup folder:

            ```bash
            POSTGRES_SECRET_MANIFEST=/root/cray-sls-postgres-backup_2021-07-07_16-39-44/cray-sls-postgres-backup_2021-07-07_16-39-44.manifest
            ```
    ...
    - From a previous automatic Postgres backup:
        ...
        1. Check for available backups:
            ...
            ```bash
            POSTGRES_SECRET_MANIFEST_NAME=cray-sls-postgres-2021-07-11T23:10:08.manifest
            POSTGRES_SQL_FILE_NAME=cray-sls-postgres-2021-07-11T23:10:08.psql
            ```
        ...
        1. Download the `.manifest` file for the SLS backup:

            ```bash
            cray artifacts get postgres-backup "$POSTGRES_SECRET_MANIFEST_NAME" "$POSTGRES_SECRET_MANIFEST_NAME"
            ```

        1. Set environment variables pointing to the full path of the `.psql` and `.manifest` files:
...
1. (`ncn-mw#`) Verify the `POSTGRES_SQL_FILE` and `POSTGRES_SECRET_MANIFEST` environment variables are set correctly:
...
1. (`ncn-mw#`) Re-run the SLS loader job:
...
1. (`ncn-mw#`) Determine leader of the Postgres cluster:
...
1. (`ncn-mw#`) Check the environment variable to see the current leader of the Postgres cluster:
...
1. (`ncn-mw#`) Determine the database schema version of the currently running SLS database and verify that it matches the database schema version from the Postgres backup:

    1. Database schema of the currently running SLS Postgres instance.
    ...
    1. Database schema version from the Postgres backup:
    ...
    1. If the database schema versions match, proceed to the next step.
       Otherwise, the Postgres backup taken is not applicable to the currently running instance of SLS.

        __WARNING__: If the database schema versions do not match, then SLS will need to be either upgraded
        or downgraded to a version with a compatible database schema version,
        ideally to the same version of SLS that was used to create the Postgres backup.

1. (`ncn-mw#`) Restore the database from the backup using the `restore_sls_postgres_from_backup.sh` script.

    This script requires the `POSTGRES_SQL_FILE` and `POSTGRES_SECRET_MANIFEST` environment variables to be set.
...
1. (`ncn-mw#`) Verify the health of the SLS Postgres cluster by running the following scripts.

    - If there are issues with `run_hms_ct_tests.sh`, then see
      [Interpreting HMS Health Check Results](../../troubleshooting/interpreting_hms_health_check_results.md).
    - If there are issues with `verify_hsm_discovery.py`, then see
      [Interpreting HSM discovery results](../validate_csm_health.md#221-interpreting-hsm-discovery-results).
...
1. (`ncn-mw#`) Verify that the service is functional:
    ...
    ```bash
    cray sls version list --format toml
    ```
...
1. (`ncn-mw#`) Get the number of hardware objects stored in SLS:

    ```bash
    cray sls hardware list --format json | jq .[].Xname | wc -l
    ```

1. (`ncn-mw#`) Get the name of networks stored in SLS:
