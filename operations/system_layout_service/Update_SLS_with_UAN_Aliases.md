...
1. Authenticate with Keycloak to obtain an API token:
   ...
   ```console
   ncn-mw# export TOKEN=$(curl -k -s -S -d grant_type=client_credentials -d client_id=admin-client \
...
1. Find the component name (xname) of the UAN by searching through all Application nodes until found.
   ...
   ```console
   ncn-mw# curl -s -k -H "Authorization: Bearer ${TOKEN}" \
...
1. Update the UAN object in SLS by adding Aliases array with the UAN's hostname.
   ...
   ```console
   ncn-mw# curl -X PUT -s -k -H "Authorization: Bearer ${TOKEN}" \
