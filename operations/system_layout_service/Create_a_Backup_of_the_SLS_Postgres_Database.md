...
* [Prerequisites](#prerequisites)
    * [Healthy SLS Postgres cluster](#healthy-sls-postgres-cluster)
    * [Healthy SLS service](#healthy-sls-service)
* [Procedure](#procedure)
...
(`ncn-mw#`) Use `patronictl list` on the SLS Postgres cluster to determine the current state of the cluster.
...
(`ncn-mw#`) Verify all 3 SLS replicas are up and running.
...
1. Display the directory where the SLS backup will be located.
...
1. Run the `backup_sls_postgres.sh` script to take a backup of the SLS Postgres:
...
1. Copy the backup folder off of the cluster, and store it in a secure location.
