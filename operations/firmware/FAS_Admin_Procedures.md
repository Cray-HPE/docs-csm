...
1. Find the available image in FAS using the command: (change *TARGETNAME* to the actual target being searched)
...
3. Run the following command to verify the correct image ID was found:
...
Rerun FAS actions command using the updated json file. **It is strongly recommended to run a dry-run (overrideDryrun=false) first and check the actions output.**
...
4. Run FAS loader to set permissions on file uploaded.
