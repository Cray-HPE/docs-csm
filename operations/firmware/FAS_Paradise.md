...
1. (`ncn#`) View more details for an operation using the `operationID` from the previous step.
    ...
    Once firmware and BIOS are updated, the compute nodes can be powered back on. If the nodes have never been powered on in the system before (they are being added during a hardware add procedure), then use the Boot Orchestration Service (BOS) to power them on.
