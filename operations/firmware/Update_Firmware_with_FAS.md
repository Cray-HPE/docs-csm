...
1. Interpret the outcome of the dry-run. Look at the counts and determine if the dry-run identified any hardware to update:
...
1. If `succeeded` count > 0, now perform a live update.

1. Update the JSON file `overrideDryrun` to `true`:
...
For example, if there are 1000 xnames with 5 targets each to be updated, then all 1000 xnames can be updating a target, but only 1 target on each xname will be updated at a time.
