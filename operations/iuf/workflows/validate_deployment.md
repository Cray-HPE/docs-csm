...
1. *post-install-service-check posthook:* When upgrading CSM, a script named `post-install-service-check-posthook.sh` runs after `post-install-service-check` stage. This hook ensures the Kubernetes upgrade is completed successfully
