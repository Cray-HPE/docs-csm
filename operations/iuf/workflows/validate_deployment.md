...
1. _post-install-service-check posthook:_ When upgrading CSM, a script named `post-install-service-check-posthook.sh` runs after `post-install-service-check` stage. This hook ensures  that the cluster is at the
expected Kubernetes version and there are no residual `.done` marker files under `/etc/cray/upgrade/csm/${CSM_REL_NAME}` after the Kubernetes upgrade is completed.
