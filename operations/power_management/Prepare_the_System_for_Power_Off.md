...
6.  Check for running sessions.
    ...
    If active sessions are running, either wait for them to complete or shut down/cancel/delete the session. See the following step.

7. Cancelling a running BOS sessions.

    1.   Identify the BOS Sessions and associated BOA Kubernetes jobs to delete
    
         Determine which BOS session or sessions you want to cancel. To cancel a BOS session, you must kill its associated Boot Orchestration Agent (BOA) Kubernetes job.

         Method #1: Use BOS Session Status

         Use this script to find the BOS session that have ended (true) or are still running (false)
         ```bash
         #! /bin/bash
         # List all of the BOS sessions. Look for ones whose status says they are not complete, i.e. still running.
         ...
         However, the BOS status output can be buggy, and it may misidentify BOS sessions as still running when they have actually finished.
	 If you only want to delete currently running jobs, then use Method #2. 
         Method #2 is a more reliable method for identifying running BOA jobs because it interacts directly with the BOA Kubernetes job.
         
         Method #2: Look at BOA Kubernetes jobs
         
         You can find a list of BOA jobs that are still running using this command.
         ```bash
    ...
    2.   Clean up prior to BOA job deletion
         ...
         Use this command to identify the BOA job's configmap.
         ```bash
         ncn-m001:/tmp # kubectl -n services describe job boa-0216d2d9-b2bc-41b0-960d-165d2af7a742 |grep ConfigMap -A 1 -B 1
            boot-session:
         ...
         Use this command to delete the configmap.
         ```bash
    ...
    3.   Delete the BOA jobs with this command.
    ...
    4.   Delete the BOS session.
	 ...
	 Note, you found the BOS Session ID earlier, but it is also invariably the same as the BOA Job ID minus the prepended 'boa-' string.
