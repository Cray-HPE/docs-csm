...
1. When reporting slow ops for OSDs, then it is good to find out if those OSDs are on the same node or different nodes.
    ...
    1. If the OSDs are on different nodes, then networking issues should be investigated.
...
1. When reporting slow ops for `MDS`, then it could be due to a couple of different reasons.
   ...
   1. If it is only listing `MDS`, then restart the MDS daemons. If the problem persists, then the logs will have to be investigated for the root cause.
