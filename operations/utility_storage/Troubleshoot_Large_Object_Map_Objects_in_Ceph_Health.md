...
Troubleshoot an issue where Ceph reports a `HEALTH_WARN` of 1 large `omap` object.
Adjust the `omap` object key threshold or number of placement groups (PG) to resolve this issue.

## Prerequisites

Ceph health is reporting a `HEALTH_WARN` for large object map (`omap`) objects.
...
1. Adjust the number of `omap` objects.
    ...
    - Increase the number of PGs for the Ceph pool.
        ...
        1. Adjust the `target_size_ratio` value to increase the PGs for the pool.

            This number should be increased a tenth or smaller at a time. Check the `autoscale-status` between each adjustment.
            When there is a change to the `NEW PG_NUM`, stop adjusting the number.
