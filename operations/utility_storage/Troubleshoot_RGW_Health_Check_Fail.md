...
- A value of `1` is returned if unable to connect to `rgw-vip`.

    This happens if any of the following three commands fail.
...
- A value of `2` is returned if a storage node is not able to be reached.

    In this case, run the `rgw_health_check.sh` as stated in the optional step above.
    Find which storage nodes are not able to be reached, and run the following checks on those nodes.
...
- A value of `3` is returned if a `craysys` command fails.

    This implies `cloud-init` is not healthy. Run the command below to determine the health.
...
- If a value of `4` or `5` is returned, then `rgw-vip` and the storage nodes are reachable.

    The error occurred when attempting to create a bucket, upload an object to a bucket, or download an object from a bucket.
    This implies that Ceph may be unhealthy. Check Ceph status with the following command.
