...
* Ensure the user account in use is an authorized user on the Cray CLI.
  ...
  Log in as a user account where the credentials are known:
   ...
   ```bash
   ncn-mw# export CRAY_CONFIG_DIR=$(mktemp -d); echo $CRAY_CONFIG_DIR; cray init --configuration default --hostname https://api-gw-service-nmn.local
   ```
...
* Get a current list of pods that have a status of anything other than `Running` or `Completed`. Investigate any of concern.
...
* Note which pods are running on an NCN that will be taken down (as well as the total number of pods running). The following is an example that shows the listing of pods running on `ncn-w001`:
   ...
   ```bash
   ncn-mw# kubectl get pods -o wide -A | grep ncn-w001 | awk '{print $2}'
   ```
...
* Verify `ipmitool` can report power status for the NCN to be shut down.
   ...
   ```bash
   linux# ipmitool -I lanplus -U root -P <password> -H <ncn-node-name> chassis power status
   ```
...
1. Set up a `watch` command to repeatedly run with the Cray CLI (that will hit the service API) to ensure that critical services can ride through a fault. Note that there is not more than a window of 5-10 minutes where a
   ...
   ```bash
   ncn-mw# watch -n 5 "date; cray cps contents"
   ```

   ```bash
   ncn-mw# watch -n 5 "date; cray bos session list"
   ```

1. Monitor Ceph health, in a window, during and after a single NCN is taken down.

   ```bash
   ncn-mw# watch -n 5 "date; ceph -s"
   ```
...
1. Identify when pods on a downed master or worker NCN are no longer responding.
   ...
   ```bash
   ncn-mw# watch -n 5 "date; kubectl get pods -o wide -A | grep Termin"
   ```

   ```bash
   ncn-mw# watch -n 10 "date; kubectl get pods -o wide -A | grep Pending"
   ```

   ```bash
   ncn-mw# watch -n 5 "date; kubectl get pods -o wide -A | grep -v Completed | grep -v Running"
   ```
...
1. Detect the change in state of the various Postgres instances running.
   ...
   ```bash
   ncn-mw# watch -n 30 "date; kubectl get postgresql -A"
   ```
...
1. Create a UAI.
   ...
   1. Create labels to ensure that UAI pods will only be able to start-up on the target worker NCN.
      ...
      ```bash
      ncn-mw# kubectl label node ncn-w001 uas=False --overwrite
      ncn-mw# kubectl label node ncn-w003 uas=False --overwrite
      ```
   ...
   1. Verify the labels were applied successfully.
      ...
      ```bash
      ncn-mw# kubectl get nodes -l uas=False
      ```
   ...
   1. Remove the labels set above after the UAI has been created.
      ...
      ```bash
      ncn-mw# kubectl label node ncn-w001 uas-
      ncn-mw# kubectl label node ncn-w003 uas-
      ```
...
1. Verify that WLM is configured with the appropriate workload manager within the created UAI.
   ...
   1. Verify the connection string of the created UAI.
      ...
      ```bash
      ncn-mw# cray uas list --format toml
      ```
...
1. Compile an MPI application with the UAI. Launch application as batch job (not interactive) on compute node(s) that have not been designated, already, for reboots once an NCN is shut down.
   ...
   1. Delete the UAI session when all of the testing is complete.
      ...
      ```bash
      ncn-mw# cray uas delete --uai-list uai-vers-f8fa541f
      ```

### Launch on a UAN

1. Log in to the UAN and verify that a WLM has been properly configured.
...
1. Reboot a pre-designated set of compute nodes and watch the reboot.
   ...
   1. Find the BOA job name using the returned BOA `jobID`.
      ...
      ```bash
      ncn-mw# kubectl get pods -o wide -A | grep <boa-job-id>
      ```

   1. Watch the progress of the reboot of the compute nodes.

      ```bash
      ncn-mw# kubectl logs -n services <boa-job-pod-name> -c boa -f
      ```
...
1. Look for any pods that are in a state other than `Running`, `Completed`, `Pending`, or `Terminating`:

   ```bash
   ncn-mw# kubectl get pods -o wide -A | grep -Ev "Running|Completed|Pending|Termin"
   ```
...
1. Use the `ipmitool` command to power up the NCN.
   ...
   ```bash
   linux# ipmitool -I lanplus -U root -P <password> -H <hostname> chassis power on   #example hostname is ncn-w003-mgmt
   ```
