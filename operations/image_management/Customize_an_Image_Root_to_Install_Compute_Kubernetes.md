## Customize an Image Root to Install Compute Kubernetes

Use the Image Management Service \(IMS\) to modify the compute image to install the base software required for Cray's Linux Environment \(CLE\) compute Kubernetes \(ck8s\) deployments. There are several starting points for modifying compute images.

An admin can start with a bare bones image as described in [Customize an Image Root Using IMS](Customize_an_Image_Root_Using_IMS.md), or start with an existing image that is already in use. This procedure starts with an image rootfs that is already being used on the compute nodes.
...
1.  Query the Boot Script Service \(BSS\) for the compute image.
    ...
    root=craycps-s3:s3://boot-images/**36e291b3-e594-4154-9e96-6975829fd9cd**/rootfs:b5ad8d47bd5d33b1fcb72a85ea70b74d-165:dvs:api-gw-service-nmn.local:300:eth0"
    initrd = "s3://boot-images/36e291b3-e594-4154-9e96-6975829fd9cd/initrd"
    hosts = [ "x3000c0s19b1n0",]
    kernel = "s3://boot-images/36e291b3-e594-4154-9e96-6975829fd9cd/kernel"
    ```

    If successful, create a variable for the root argument in the returned output that references the boot image ID. In the example above, this value is 36e291b3-e594-4154-9e96-6975829fd9cd.

    ```bash
    ncn# export ROOTFS=36e291b3-e594-4154-9e96-6975829fd9cd
    ```
...
2.  Find the image that refers to the desired artifact.
    ...
    id = "36e291b3-e594-4154-9e96-6975829fd9cd"
    name = "cle_default_rootfs_cfs_469dbeb4-16b0-11ea-a729-a4bf01560700"
    
    [results.link]
    type = "s3"
    path = "/36e291b3-e594-4154-9e96-6975829fd9cd/cle_default_rootfs_cfs_469dbeb4-16b0-11ea-a729-a4bf01560700"
    etag = ""
    ```
    ...
    ```bash
    ncn# export IMS_IMAGE_ID=36e291b3-e594-4154-9e96-6975829fd9cd
    ```
...
6.  Create an IMS job record and start the image customization job.
    ...
    --job-type customize \
    --image-root-archive-name ck8s_image \
    ...
    image_root_archive_name = "ck8s_image"
...
9.  Change to the image root directory.
    ...
    Once connected to the IMS image customization shell, customizations to install ck8s can be made. If the SSH shell was created without using the --ssh-containers-jail True parameter, cd or chroot into the image root.
...
11. Install ck8s components.

    ```bash
    :/ # zypper refresh
    :/ # zypper --non-interactive --no-gpg-checks install singularity \
    cray-docker-sup oss-docker-ce oss-kube-artifacts oss-kubespray sudo python3-netaddr
    :/ # zypper --non-interactive --no-gpg-checks install \
    http://ncn-w001:8888/shasta-cd-repo/bloblets/os/rpms/opensuse/15/external/noarch/ansible-2.9.6-lp151.2.7.1.noarch.rpm
    ```

12. Finish the install and exit the image customization environment.
...
13. Ensure that any artifacts are properly uploaded to S3 and associated with IMS.
    ...
    + python -m ims_python_helper image upload_artifacts ck8s_image 7de80ccc-1e7d-43a9-a6e4-02cad10bb60b 
    -v -r /mnt/image/ck8s_image.sqsh -k /mnt/image/image-root/boot/vmlinuz 
...
14. Look up the ID of the newly created image.
...
15. Verify the new IMS image record exists.

    ```bash
    ncn# cray ims images describe $IMS_RESULTANT_IMAGE_ID
    created = "2018-12-04T17:25:52.482514+00:00"
    id = "d88521c3-b339-43bc-afda-afdfda126388"
    name = "ck8s_image.squashfs"
    
    [link]
    type = "s3"
    path = "/d88521c3-b339-43bc-afda-afdfda126388/ck8s_image.squashfs"
    etag = "28f3d78c8cceca2083d7d3090d96bbb7"
    ```

16. Look up the IMS Job ID.
