...
* The `tenant` field indicates which tenant owns this session template.
    * If this field is not present, or if it has an empty or null value, then the template is not owned by a tenant.
...
The `arch` field specifies the hardware architecture of the target nodes.
Supported values are `X86` and `ARM`. If this field is not present in a boot set, it defaults to `X86`.
