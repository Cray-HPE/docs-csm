...
BOS data can be backed up using an automated script. BOS session templates and options can be restored from the archive files produced by
...
1. (`ncn-mw#`) Run the following script to perform the export of all BOS data (including session templates and options).
...
BOS session templates or BOS options can also be manually exported and imported onto a given system using the Cray CLI tool.
...
1. (`ncn-mw#`) Save the current BOS options on the system.
...
1. (`ncn-mw#`) The CLI can also be used to change BOS options to match the values that were exported.
