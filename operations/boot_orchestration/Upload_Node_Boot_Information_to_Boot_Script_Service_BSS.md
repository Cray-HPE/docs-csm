...
1. Set `KERNEL` to the [S3][s3] download URL of the kernel artifact.

    This should be in the `s3://s3_BUCKET/S3_OBJECT_KEY/kernel` format.

    ```console
    ncn-mw# KERNEL=s3://boot-images/97b548b9-2ea9-45c9-95ba-dfc77e5522eb/kernel
    ```

1. Set `INITRD` to the [S3][s3] download URL of the `initrd` artifact.

    This should be in the `s3://s3_BUCKET/S3_OBJECT_KEY/initrd` format.

    ```console
    ncn-mw# INITRD=s3://boot-images/97b548b9-2ea9-45c9-95ba-dfc77e5522eb/initrd
    ```

1. Set `ROOTFS` to the [S3][s3] download URL of the `rootfs` artifact.

    This should be in the `s3://s3_BUCKET/S3_OBJECT_KEY/rootfs` format.

    ```console
    ncn-mw# ROOTFS=s3://boot-images/97b548b9-2ea9-45c9-95ba-dfc77e5522eb/rootfs
    ```

1. Set `ETAG` to the `etag` of the `rootfs` in [S3][s3].

    ```console
    ncn-mw# S3_BUCKET_KEY=$(echo "${ROOTFS}" | sed 's#^s3://\([^/]\+\)/\(.*rootfs\)$#\1 \2#')
    ncn-mw# ETAG=$(cray artifacts describe ${S3_BUCKET_KEY} --format json | jq -r '.artifact.ETag' | tr -d '"')
    ```

1. Set `PARAMS` to the boot kernel parameters.
    ...
    **IMPORTANT:** The `PARAMS` line must always include the substring `crashkernel=512M`.
    ...
    ```console
    ncn-mw# PARAMS="console=ttyS0,115200 bad_page=panic crashkernel=512M,high "
    ncn-mw# PARAMS+="crashkernel=256M,low intel_pstate=disable numa_balancing=disable oops=panic "
    ncn-mw# PARAMS+="pcie_ports=native rd.retry=10 rd.shell split_lock_detect=off systemd.unified_cgroup_hierarchy=1 "
    ncn-mw# PARAMS+="ip=dhcp quiet spire_join_token=\${SPIRE_JOIN_TOKEN} "
    ncn-mw# PARAMS+="root=craycps-s3:${ROOTFS}:${ETAG}:dvs:api-gw-service-nmn.local:300:eth0 "
    ncn-mw# PARAMS+="nmd_data=url=${ROOTFS},etag=${ETAG} bos_update_frequency=4h"
    ```

1. Review the variables and verify that the values are correct.

    ```console
    ncn-mw# echo "KERNEL=${KERNEL}"
    ncn-mw# echo "INITRD=${INITRD}"
    ncn-mw# echo "ROOTFS=${ROOTFS}"
    ncn-mw# echo "ETAG=${ETAG}"
    ncn-mw# echo "PARAMS=${PARAMS}"
    ```
...
1. Set `HOSTS` to a comma-separated list of the node component names ([xnames][xname])
   whose [BSS][bss] entries should be updated.

    ```console
    ncn-mw# HOSTS=x3000c0s21b1n0,x3000c0s21b2n0
    ```

1. Create the boot parameters in [BSS][bss] for the selected nodes.

    ```console
    ncn-mw# cray bss bootparameters create --hosts "${HOSTS}" --kernel "${KERNEL}" --initrd "${INITRD}" --params "${PARAMS}"
    ```

1. Confirm that the information has been uploaded to [BSS][bss].

    ```console
    ncn-mw# cray bss bootparameters list --hosts "${HOSTS}"
    ```

#### Update BSS by NID

1. Set `NIDS` to a comma-separated list of the [node IDs][nid] whose [BSS][bss] entries should be updated.

    ```console
    ncn-mw# NIDS=1001,1032
    ```

1. Create the boot parameters in [BSS][bss] for the selected nodes.

    ```console
    ncn-mw# cray bss bootparameters create --nids "${NIDS}" --kernel "${KERNEL}" --initrd "${INITRD}" --params "${PARAMS}"
    ```

1. Confirm that the information has been uploaded to [BSS][bss].

    ```console
    ncn-mw# cray bss bootparameters list --nids "${NIDS}"
    ```
...
- View the boot script in [BSS][bss] using a [NID][nid].

    ```console
    ncn-mw# cray bss bootscript list --nid NODE_ID
    ```

- View the boot script in [BSS][bss] using a host name.

    ```console
    ncn-mw# cray bss bootscript list --name HOST_NAME
    ```

### View all BSS contents

View the entire contents of [BSS][bss].
...
View the information that [BSS][bss] retrieved from the
...
View all boot parameter information in [BSS][bss].
...
[check-latest-docs]: ../../update_product_stream/index.md#check-for-latest-workarounds-and-documentation-updates
