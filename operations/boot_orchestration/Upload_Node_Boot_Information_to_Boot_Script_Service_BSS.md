...
1. Set `KERNEL` to the [S3](../../glossary.md#simple-storage-service-s3) download URL of the kernel artifact.
...
1. Set `INITRD` to the [S3](../../glossary.md#simple-storage-service-s3) download URL of the `initrd` artifact.
...
1. Set `ROOTFS` to the [S3](../../glossary.md#simple-storage-service-s3) download URL of the `rootfs` artifact.
...
1. Set `ETAG` to the `etag` of the `rootfs` in [S3](../../glossary.md#simple-storage-service-s3).
...
1. Set `HOSTS` to a comma-separated list of the node component names ([xnames](../../glossary.md#xname))
...
1. Create the boot parameters in BSS for the selected nodes.
...
1. Set `NIDS` to a comma-separated list of the [node IDs](../../glossary.md#node-id-nid) whose BSS entries should be updated.
...
- View the boot script in BSS using a NID.

    ```console
    ncn-mw# cray bss bootscript list --nid NODE_ID
    ```

- View the boot script in BSS using a host name.
...
View the entire contents of BSS.
...
View the information that BSS retrieved from the
...
View all boot parameter information in BSS.
