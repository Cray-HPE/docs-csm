...
The BOS reporter is a service that runs on all components managed by BOS.
It reports to BOS the current state and boot artifacts for the node on which it is running.
This is necessary for BOS to be able to manage a node. For example, it is required in order for
BOS to know that a node has successfully booted into the correct state.
