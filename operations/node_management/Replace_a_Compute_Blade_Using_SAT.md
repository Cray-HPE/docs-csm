...
Power off the slot and delete blade information from HSM.

(`ncn-mw#`) Use the `sat swap` command to power off the slot and delete the blade's Ethernet interfaces and Redfish endpoints from HSM.
...
(`ncn-mw#`) Use the `sat swap` command to begin discovery for the blade and add it to hardware management.
