...
1. Use Boot Orchestration Services (BOS) to shut down the affected nodes. Specify the appropriate BOS template for the node type.
   ...
   ```bash
   ncn-mw# cray bos v1 session create --template-uuid BOS_TEMPLATE \
               --operation shutdown --limit x1000c3s0b0n0,x1000c3s0b0n1,x1000c3s0b1n0,x1000c3s0b1n1
   ```

   Specify all the nodes in the blade using a comma-separated list. This example shows the command to shut down an EX425 compute blade (Windom) in cabinet 1000, chassis 3, slot 5.
   This blade type includes two node cards, each with two logical nodes (4 processors).
...
1. Disable the chassis slot in the Hardware State Manager (HSM).
   ...
   1. Suspend the `hms-discovery` cronjob to prevent slot power on.

      ```bash
      ncn-mw# kubectl -n services patch cronjobs hms-discovery -p '{"spec" : {"suspend" : true }}'
      ```

   1. Verify that the `hms-discovery` cronjob has stopped (`ACTIVE` column = `0`).
...
1. Delete the node Ethernet interface MAC addresses and the Redfish endpoint from the Hardware State
   ...
   **IMPORTANT**: The HSM stores the node's BMC NIC MAC addresses for the hardware management
   ...
   1. **For each** node delete the node's NIC MAC addresses from the HSM Ethernet interfaces table.
      ...
      1. Delete each node NIC MAC address in the Hardware State Manager (HSM) Ethernet interfaces table.
...
1. Replace the blade hardware.
   ...
   For detailed instructions, review the *Remove a Compute Blade Using the Lift* procedure in *HPE Cray EX Hardware Replacement Procedures H-6173* at [HPE Support](https://internal.support.hpe.com/).
...
1. Enable MEDS for the compute node(s) in the blade.
   ...
   ```bash
   ncn-mw# cray hsm inventory redfishEndpoints update --enabled true --rediscover-on-update true
   ```

   The updated component names (xnames) will be returned.
...
1. Verify that the BMCs have been discovered by the HSM.
...
1. Optional: If necessary, update the firmware.

    Review the [Firmware Action Service (FAS)](../firmware/FAS_Admin_Procedures.md) documentation.
...
1. Use boot orchestration to power on and boot the nodes.
    ...
    ```bash
    ncn-mw# cray bos v1 session create --template-uuid BOS_TEMPLATE --operation reboot \
