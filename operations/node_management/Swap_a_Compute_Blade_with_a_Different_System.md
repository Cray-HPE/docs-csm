...
- The blades must have their coolant drained and replaced to avoid cross-contamination of cooling systems. 
  - Review procedures in *HPE Cray EX Coolant Service Procedures H-6199* 
  - Review the *HPE Cray EX Hand Pump User Guide H-6200*

The two systems in this procedure are referred to as:

- System A  - a Cray EX TDS cabinet x9000 EX425 blade (Windom) in chassis 3, slot 0
- System B - a Cray EX cabinet x1005 an EX425 blade (Windom) in chassis 3, slot 0
...
4. Remove the system specific settings from each node controller on the blade.
   ...
   Alternatively, SSH to the node controller:
...
**IMPORTANT**: Record the node management network (NMN) MAC and IP addresses for each node in the blade in System A.  To avoid disruptions in the data virtualization service (DVS), these addresses must be applied to the nodes from System B when that blade is installed in System A. 

The hardware management network MAC and IP addresses are assigned algorithmically and *must not be deleted* from the HSM.

7. Query HSM to determine the NMN MAC and IP addresses for each node in the blade. (On Windom blades this command must run for b0n0, b0n1, b1n0, b1n1.)
   ...
   2. Repeat the preceding command to record the MAC and IP addresses for a the nodes in the blade.

   3. Delete the NMN MAC and IP addresses from the HSM for each node. (Each Windom node has 2 NICs).
...
9. Remove the coolant from the blade and replaced with fresh coolant to avoid cross-contamination of cooling systems. 
10. Install the blade in a storage rack.  
   ...
   - Review fill stations procedures in *HPE Cray EX Coolant Service Procedures H-6199*. If using the hand pump, review procedures in the *HPE Cray EX Hand Pump User Guide H-6200*.

### System B Procedure

9. Use WLM remove jobs from the affected nodes on the blade.

10. Use BOS to shut down the affected nodes on the blade (in this example, x1005c3s0). 
...
11. Temporarily disable endpoint discovery service (MEDS) for the compute nodes(s). 
...
**IMPORTANT**: Record the node management network NIC MAC and IP addresses for each node in the blade in System B.  To avoid disruptions in the data virtualization service (DVS), these addresses must be applied to the nodes from System A when that blade is installed in System B.  
...
15. Query HSM to determine the NMN NIC IP and MAC addresses associated with the blade in System B. The following command must run for each node on a Windom blade (4 times.) 
    ...
    2. Delete the node NIC MAC addresses from the HSM Ethernet interfaces table for each node.
    ...
    3. Delete the Redfish endpoints for each node.
...
#### Update the MAC and IP addresses for the nodes on System B

16. Add the MAC and IP addresses recorded from the System A to the System B hardware state manager (HSM). For Windom blades, there will be four MAC addresses and IP addresses.
...
17. Remove the blade hardware from System B.
18. Replace the coolant in the blade with fresh coolant to avoid cross-contamination of cooling systems. Review procedures in *HPE Cray EX Coolant Service Procedures H-6199*. To use the hand pump to fill/drain modules, review the *HPE Cray EX Hand Pump User Guide H-6200*.
19. Install the blade from System B in System A.
20. Install the blade from System A in System B.

#### Update the IP and MAC addresses for the nodes on System A

18. Update the MAC and IP addresses recorded from System B to the HSM on System A see Step 16).
    *** * * *DO WE DO THIS STEP HERE? * * ***


#### Discover the nodes on System A
...
#### Power on and boot the nodes in System B
