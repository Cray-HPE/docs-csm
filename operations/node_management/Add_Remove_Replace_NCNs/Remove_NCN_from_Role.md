...
1. **For `ncn-m001` only**: Collect and record the BMC IP address for `ncn-m001` and the CAN IP address for `ncn-m002`.
   ...
   1. Record the CAN IP address for `ncn-m002`.
      ...
      ```bash
      ncn# CAN_IP=$(ssh ncn-m002 ip addr show vlan007 | grep "inet " | awk '{print $2}' | cut -f1 -d'/')
      ncn# echo ${CAN_IP}
      ```
