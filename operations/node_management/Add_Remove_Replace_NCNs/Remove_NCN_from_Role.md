...
1. [Wipe the drives](#2-wipe-the-drives)
...
1. Fetch the defined `first-master-hostname`.
    ...
    ```bash
    ncn-mw# cray bss bootparameters list --hosts Global --format json |jq -r '.[]."cloud-init"."meta-data"."first-master-hostname"'
    ```
...
1. Configure the newly promoted first master node so it is able to have other nodes join the cluster.
   ...
   1. Copy/paste the following script to a file, and then execute it.
...
The master node role removal is complete. Proceed to [Wipe the drives](#2-wipe-the-drives).
...
The worker node role removal is complete. Proceed to [Wipe the drives](#2-wipe-the-drives).
...
**NOTE:** etcd should already be stopped as part of the [Remove roles](#1-remove-roles) steps.
...
Once the wipe of the drives is complete, proceed to [Power off the node](#3-power-off-the-node).
...
1. **For `ncn-m001` only**: Collect and record the BMC IP address for `ncn-m001` and the CMN IP address for `ncn-m002`.
   ...
   1. Record the CMN IP address for `ncn-m002`.
      ...
      ```bash
      ncn# CMN_IP=$(ssh ncn-m002 ip -4 a show bond0.cmn0 | grep inet | awk '{print $2}' | cut -d / -f1)
      ncn# echo ${CMN_IP}
      ```
...
1. Set and export the `root` user password of the BMC.
