...
1. (`ncn-mw#`) Restart `keycloak-gatekeeper` to pick up the newly generated client ID.
   ...
   1. Wait for the restart to complete.
      ...
      ```bash
      kubectl rollout status deployment -n services cray-keycloak-gatekeeper-ingress
      ```
