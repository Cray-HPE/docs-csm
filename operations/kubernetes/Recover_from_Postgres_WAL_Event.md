...
then skip to [Rebuild the cluster and restore the data](#3-rebuild-the-cluster-and-restore-the-data).
...
1. [Attempt to recover to a running database](#1-attempt-to-recover-to-a-running-database)
...
1. [Dump the data](#2-dump-the-data)
1. [Rebuild the cluster and restore the data](#3-rebuild-the-cluster-and-restore-the-data)
...
1. Clear files from `/home/postgres/pgdata/pgroot/pg_log/` until the database is running again and allowing connections.
   ...
   For example, if the disk space is at 100%, then copy the logs off (optional), exec into the pod, and then clear the logs to recover some disk space.
    ...
    1. (`postgres-container#`) Clear the logs.

        ```bash
        for i in {0..7}; do > /home/postgres/pgdata/pgroot/pg_log/postgresql-$i.csv; done
        ```

1. (`ncn-mw#`) Restart the Postgres cluster and `postgres-operator`.
...
1. (`ncn-mw#`) Check if the database is running. If it is running, then proceed to [Dump the data](#2-dump-the-data).
...
1. If the database is still not running, then delete files from `/home/postgres/pgdata/pgroot/data/pg_wal/`.
    ...
    1. (`postgres-container#`) Delete the files.
...
1. (`ncn-mw#`) Edit `numberOfInstances` in the `postgresql` resource from 3 to 1.
...
1. (`ncn-mw#`) Wait for 2 of the 3 `postgresql` pods to terminate.
...
1. (`ncn-mw#`) Delete the PVCs from the non-running Postgres pods.
...
1. (`ncn-mw#`) Resize the remaining Postgres PVC `resources.requests.storage` to `$PGRESIZE`.
...
1. (`ncn-mw#`) Wait for the PVC to resize.
...
1. (`ncn-mw#`) Update the `postgresql` resource `spec.volume.size` to `$PGRESIZE`.
...
1. (`ncn-mw#`) Restart the existing `postgresql` pod.
...
1. (`ncn-mw#`) Perform verifications.
...
1. (`ncn-mw#`) Scale `numberOfInstances` in `postgresql` resource from 1 back to 3.
...
1. (`ncn-mw#`) Logs may indicate WAL error such as the following, but a dump can be taken at this point.
...
1. (`ncn-mw#`) Record the number of replicas the client service is using. and then scale it to 0.
...
1. (`ncn-mw#`) Wait for the running pods to terminate.
...
1. (`ncn-mw#`) Delete and re-create the `postgresql` resource (which includes the PVCs).
...
1. (`ncn-mw#`) Record the name of the leader.

    ```bash
    POSTGRES_LEADER=cray-smd-postgres-0
    ```

1. (`ncn-mw#`) Copy the dump taken earlier to the Postgres leader pod and restore the data.
...
1. Restore the secrets.
    ...
    1. For each secret above, get the username and password from Kubernetes and update the Postgres database with this information.
        ...
        1. (`ncn-mw#`) Open an interactive Postgres console in the leader container.

            ```bash
            kubectl exec ${POSTGRES_LEADER} -n ${NAMESPACE} -c postgres -it -- /usr/bin/psql postgres postgres
            ```

        1. (`postgres#`) Update the password for the user.
        ...
        1. (`postgres#`) If this is the last user being updated, enter `\q` to exit the console.

    1. Continue the above process until all `${POSTGRESQL}.credentials` secrets have been updated in the database.

1. (`ncn-mw#`) Restart the `postgresql` cluster and wait for it to start running.
    ...
    1. Wait for it to start running.
...
1. (`ncn-mw#`) Scale the client service back to the original number of replicas and wait for it to start running.
