...
1. Upload NCN artifacts, filling `CSM_RELEASE` with the actual release tarball.
   ...
   artdir=/var/www/ephemeral/${CSM_RELEASE}/images
...
1. Upload the bootstrap information; note this denotes information that should always be kept together in order to fresh-install the system again.
   ...
   - **Option 1**: Copy to ncn-m002
        ...
        - Login; setup passwordless SSH _to_ the pit node by copying ONLY the public key from `ncn-m002` to the `pit` (**do not setup passwordless SSH _from_ the PIT** or the key will have to be securely tracked or expunged if using a USB installation).
        ...
        - Login to ncn-m002, then into pit.nmn; accept the host signatures to allow the upcoming nested rsync to work:

            ```bash
            pit# ssh ncn-m002 ssh pit.nmn
            ```

        - Run this to create the backup; in one swoop, login to m002 and pull the files off the pit. _This runs `rsync` with specific parameters; `partial`, `non-verbose`, and `progress`._
      ...
      - Handoff prep and CSM backups are done; the administrator or CI/CD agent can move onto the next step.
...
1. Collect a backdoor login ... fetch the CAN IP for ncn-m002 for a backdoor during the reboot of ncn-m001.
    ...
    - Login from another external machine to verify SSH is up and running for this session.
       ```bash
...
1. Quit the typescript session with the `exit` command and copy the file (booted-csm-lived.<date>.txt)
...
1. Login and start a typescript (the IP used here is the same from step 9).
   - Login to ncn-m002 using the IP we collected earlier and setup a typescript.
      ```bash
      external# ssh root@10.102.11.13
...
1. Run `kubectl get nodes` to see the full Kubernetes cluster.
   ...
   ncn-m002   Ready    <none>   4h39m   v1.18.6
...
1. Install that latest the documentation and workaround packages. This will require external access. 
...
1. Obtain access to CSI
   (run this from `ncn-m001`)
   ```bash
   export CSM_RELEASE=csm-x.y.z
   mkdir -pv /mnt/livecd /mnt/rootfs /mnt/sqfs
   mount /metal/bootstrap/csm*/cray-pre-install-toolkit-*.iso /mnt/livecd/
...
- [CSM Validation](008-CSM-VALIDATION.md)
