...
leader pod $leader ---"
            ...
            kubectl logs -n $c_ns $leader postgres | \
                awk '{$1="";$2=""; print $line}' | egrep "ERROR" \
                | egrep -v "NewConnection|bootstrapping|get_cluster|IncompleteRead\(0 bytes read\)" | sort -u
        fi
...
echo
echo "--- Postgres Health Check Finished ---"
echo
echo "If 'ERRORS' are found in any of the posgres pod error logs above, manually examine the full logs for the pod using the command below."
echo "ncn-m001:~ # kubectl logs -n <namespace> <pod> postgres | awk '{\$1=\"\";\$2=\"\"; print \$line}'\
| egrep \"INFO|ERROR\" | egrep -v \"NewConnection|bootstrapping|get_cluster|IncompleteRead\(0 bytes read\)\" \
| sort -u"
exit 0;
