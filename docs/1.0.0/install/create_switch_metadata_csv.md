...
1. Identify the switches in the SHCD.
   ...
   Look for the following:
    - The slot numbers for the leaf switches (usually 48-port switches)
...
1. Each spine or aggregation switch will follow this format: `xXcChHsS`:
...
1. Each leaf switch will follow this format: `xXcCwW`:
...
1. Each CDU switch will follow this format: `dDwW`:
    ...
    > If a CDU switch is in a River cabinet, then follow the naming convention in earlier "spine or aggregation switch" step instead.

    - `dD` : where `D` is the Coolant Distribution Unit (CDU)
    - `wW` : where `W` is the management switch in a CDU

1. Each line in the file must specify the type: `Aggregation`, `CDU`, `Leaf`, or `Spine`.
...
## Example `switch_metadata.csv` files

> __Example:__ Two Aruba CDU switches, two Aruba leaf switches, four Aruba aggregation switches, and two Aruba spine switches:
...
> __Example:__ Two Dell CDU switches, two Dell leaf switches, and two Mellanox spine switches:
...
> __Example:__ Two Dell leaf switches and two Mellanox switches in the same slot number:
