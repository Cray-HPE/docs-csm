...
1. (`ncn-m001#`) Start a typescript.
    ...
    ```bash
    script -af /root/csm_upgrade.$(date +%Y%m%d_%H%M%S).stage_1_ncn-m001.txt
...
## Stage 1.1 - Master node image upgrade
...
Before starting [Stage 1.2 - Worker node image upgrade](#stage-12---worker-node-image-upgrade), access the Argo UI to view the progress of this stage.
...
## Stage 1.2 - Worker node image upgrade
...
1. (`ncn-m002#`) Start a typescript.
    ...
    ```bash
    script -af /root/csm_upgrade.$(date +%Y%m%d_%H%M%S).stage_1_ncn-m002.txt
    export PS1='\u@\H \D{%Y-%m-%d} \t \w # '
    ```
...
## Stage 1.5 - `coredns` anti-affinity
...
## Stage 1.6 - Complete Kubernetes upgrade
...
> **REMINDER**: If password for `ncn-m002` was reset during Stage 1.3, then also reset the password
