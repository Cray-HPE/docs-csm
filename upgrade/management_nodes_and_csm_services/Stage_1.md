...
1. Run `ncn-upgrade-ceph-nodes.sh` for `ncn-s001`. Follow output of the script carefully. The script will pause for manual interaction.
    ...
    ```bash
    ncn-m001# /usr/share/doc/csm/upgrade/management_nodes_and_csm_services/scripts/upgrade/ncn-upgrade-ceph-nodes.sh ncn-s001
    ```
...
1. Update BSS to ensure that the Ceph images are loaded if a node is rebuilt.
    ...
    ```bash
    ncn-m001# . /usr/share/doc/csm/upgrade/management_nodes_and_csm_services/scripts/ceph/lib/update_bss_metadata.sh
