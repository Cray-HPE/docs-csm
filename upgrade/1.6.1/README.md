# CSM 1.6.1 Patch Installation Instructions
...
> _In the unusual situation of upgrading from a pre-release version of CSM `v1.6.0`, then follow the procedures described in [CSM major/minor version upgrade](../README.md#csm-majorminor-version-upgrade) instead._
...
* Add ability to SAT to sort reports by multiple fields
* Cleanup previous/old SquashFS images during upgrade
* Drop internal default values for `rootfs_provider` and `rootfs_provider_passthrough` from sat bootprep
* Add context managers around BOS requests/sessions; enable paging of BOS components
* Update `customizations.yaml` for SMA victoria metrics PVC size
...
1. Download and extract the CSM `v1.6.1` release to `ncn-m001`.
...
1. (`ncn-m001#`) Set `CSM_DISTDIR` to the directory of the extracted files.
   ...
   **IMPORTANT**: If necessary, change this command to match the actual location of the extracted files.
   ...
   ```bash
   export CSM_DISTDIR="$(pwd)/csm-1.6.1"
   echo "${CSM_DISTDIR}"
   ```
...
1. Verify that the new CSM version is in the product catalog.
   ...
   Example output that includes the new CSM version (`1.6.1`):
...
1. Confirm that the product catalog has an accurate timestamp for the CSM upgrade.
   ...
   ```bash
   kubectl get cm cray-product-catalog -n services -o jsonpath='{.data.csm}' | yq r  - '"1.6.1".configuration.import_date'
   ```
