...
> - Refer to [Storage node `cloud-init` fails with 'Timed out waiting for device' error](../troubleshooting//known_issues/storage_node_cloud_init_fails_with_timed_out.md) if `cloud-init` is failing on the storage node.
