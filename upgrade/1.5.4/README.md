...
This document guides an administrator through the patch update to Cray Systems Management `v1.5.4`
...
* Fixed issue with false positive goss test failure ([`CASMPET-7088`](https://jira-pro.it.hpe.com:8443/browse/CASMPET-7088) Bo Quan)
* Fixed bug in `cfs-debugger` ([`CASMCMS-9247`](https://jira-pro.it.hpe.com:8443/browse/CASMCMS-9247) - Mitch Harding)
* Fixed bug when validating a non-existent session template ([`CASMCMS-9270`](https://jira-pro.it.hpe.com:8443/browse/CASMCMS-9270) - Mitch Harding)
...
1. Download and extract the CSM `v1.5.4` release to `ncn-m001`.
...
1. (`ncn-m001#`) Set `CSM_DISTDIR` to the directory of the extracted files.
   ...
   **IMPORTANT**: If necessary, change this command to match the actual location of the extracted files.
   ...
   ```bash
   export CSM_DISTDIR="$(pwd)/csm-1.5.4"
   echo "${CSM_DISTDIR}"
   ```
...
1. Verify that the new CSM version is in the product catalog.
   ...
   Example output that includes the new CSM version (`1.5.4`):
...
1. Confirm that the product catalog has an accurate timestamp for the CSM upgrade.
   ...
   ```bash
   kubectl get cm cray-product-catalog -n services -o jsonpath='{.data.csm}' | yq r  - '"1.5.4".configuration.import_date'
   ```
