...
function check_service(){
  ...
  if [[ $service =~ "osd" ]]
    then
      ...
      then
       ...
       then
         ...
         read -p "Press 'y' to attempt to fix it, press 'w' to pause the install so it can be manually fixed, press 'x' to exit the install."
         case "${REPLY}"  in
         y)
           ceph orch system start $osd;;
         w)
           read -p "Pausing until enter/return it pressed";;
         x)
           exit 1;;
         esac
       fi
       ...
       echo "confirming the container is running"
       if [[ -n "$osd" ]]
      then
    ...
    for mds in $(ceph orch ps --daemon_type mds --hostname $host -f json-pretty |jq -r '.[]|(.daemon_type+"."+.daemon_id)')
        ...
        echo "$mds is_active: $(ceph orch ps --daemon_type mds --hostname $host -f json-pretty|jq -r '.[].is_active')"
        echo "confirming the container is running"
        read -d "\n" service_unit status epoch < <(pdsh -N -w $host podman ps --format json |jq --arg service $service -r '.[]|select(.Names[]|contains($service))|.Names[], .State, .StartedAt')
        echo "Service unit name: $service_unit"
        echo "Status: $status"
